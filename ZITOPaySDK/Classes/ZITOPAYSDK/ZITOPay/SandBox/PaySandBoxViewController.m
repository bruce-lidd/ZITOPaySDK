...
- (void)back {
    resultCode = ZITOErrCodeUserCancel;
    resultMsg = @"支付取消";
...
- (void)doNotifyResponse:(NSDictionary *)response {
    [loading stopAnimating];
    if ([response integerValueForKey:kKeyResponseResultCode defaultValue:ZITOErrCodeCommon] == ZITOErrCodeSuccess) {
        resultMsg = @"支付成功";
        resultCode = ZITOErrCodeSuccess;
    }
    [self dismissViewControllerAnimated:YES completion:^{
